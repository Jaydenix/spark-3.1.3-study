...
class SparkContext(config: SparkConf) extends Logging {
  ...
  try {
    logInfo("=====3.成员变量包括：SchedulerBackend TaskScheduler DAGScheduler =====")
    _conf = config.clone()
    ...
    // 创建SparkEnv对象
    logInfo("=====3.创建SparkEnv SparkUI对象=====")
    ...
    logInfo("=====创建心跳接收器HeartbeatReceiver=====")
    ...
    // 创建TaskScheduler,如果master==local.. 就是StandaloneSchedulerBackend 它是CoarseGrainedSchdulerBackend的子类
    logInfo("-----3.<开始>创建TaskScheduler SchedulerBackend-----")
    val (sched, ts) = SparkContext.createTaskScheduler(this, master, deployMode)
    logInfo("-----3.<结束>创建TaskScheduler SchedulerBackend-----")
    _schedulerBackend = sched
    _taskScheduler = ts
    // 创建DAGScheduler
    logInfo("#####<开始>创建 DAGScheduler#####")
    _dagScheduler = new DAGScheduler(this)
    logInfo("#####<结束>创建 DAGScheduler#####")
    ...
    // constructor
    // 开启任务调度器,内部开启了SchedulerBackend
    logInfo("-----3.启动TaskScheduler-----")
    _taskScheduler.start()
    logInfo("-----3.TaskScheduler启动完毕-----")
  ...
  def runJob[T, U: ClassTag](
    ...
    logInfo("#####SparkContext初始化完毕之后,真正开始运行job#####\n \n \n")
    logInfo("Starting job: " + callSite.shortForm)
    if (conf.getBoolean("spark.logLineage", false)) {
      logInfo("RDD's recursive dependencies:\n" + rdd.toDebugString)
    }
    // 提交到dagScheduler中
    logInfo("=====将job提交到DAGScheduler中=====")
...
object SparkContext extends Logging {
  ...
  private def createTaskScheduler(
    ...
    /*
    * 根据--master的配置来决定使用哪种调度器(TaskSchedulerImpl中的SchedulerBackend类型)
    * 这与之后的任务调度
    * local/local[*]/local[*,*] -> LocalSchedulerBackend
    * URL类似于spark://masterIP -> StandaloneSchedulerBackend
    * local-cluster[*,*,*] -> StandaloneSchedulerBackend
    * 外部的集群管理器:yarn/k8s... ->集群管理器内部指定的SchedulerBackend
    * */
    master match {
      case "local" =>
      ...
      case SPARK_REGEX(sparkUrl) =>
        logInfo(s"=====3.识别出当前为StandAlone模式=====")
        logInfo(s"#####<开始>创建TaskSchedulerImpl#####")
        val scheduler = new TaskSchedulerImpl(sc)
        logInfo(s"#####<结束>创建TaskSchedulerImpl#####")
        val masterUrls = sparkUrl.split(",").map("spark://" + _)
        logInfo(s"#####<开始>创建StandaloneSchedulerBackend对象#####")
        val backend = new StandaloneSchedulerBackend(scheduler, sc, masterUrls)
        logInfo(s"#####<结束>StandaloneSchedulerBackend对象创建#####")
        logInfo(s"=====<开始>使用StandaloneSchedulerBackend初始化TaskSchedulerImpl=====")
        scheduler.initialize(backend)
        logInfo(s"=====<结束>使用StandaloneSchedulerBackend初始化TaskSchedulerImpl=====")
        (backend, scheduler)
