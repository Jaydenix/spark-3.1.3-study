  ...
  override def onStart(): Unit = {
    driverArgs.cmd match {
      case "launch" =>
        ...
        // 传递过去的command中 第一句就是jar org.apache.spark.deploy.worker.DriverWrapper，也就是创建的是DriverWrapper进程
        ...
        val driverDescription = new DriverDescription(
          ...
          driverResourceReqs)
        //消息1和消息3
        // 消息1：向master发送RequestSubmitDriver消息，请求开启Driver；跳转到Master当中的receiveAndReply方法
        // 消息3：期望回复SubmitDriverResponse消息(driver已创建)
        logInfo("=====消息1：client客户端请求开启driver进程(DriverWrapper),期望回复为消息3(SubmitDriverResponse)=====")
