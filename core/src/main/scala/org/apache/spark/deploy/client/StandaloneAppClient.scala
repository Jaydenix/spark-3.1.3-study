...
private[spark] class StandaloneAppClient(
  ...
  // 该类继承了ThreadSafeRpcEndpoint的RpcEndpoint特性，一定会触发RpcEndpoint的生命周期
  // constructor -> onStart -> receive* -> onStop
  private class ClientEndpoint(override val rpcEnv: RpcEnv) extends ThreadSafeRpcEndpoint
    with Logging {
    logInfo("=====该类包含ClientEndpoint类,继承了ThreadSafeRpcEndpoint的RpcEndpoint特性=====")
    logInfo("=====一定会触发RpcEndpoint的生命周期：constructor -> onStart -> receive* -> onStop=====")
    ...
    // RpcEndpoint一定会调用onStart()
    override def onStart(): Unit = {
      logInfo("=====进入StandaloneAppClient重写的OnStart方法=====")
      try {
        // 向master注册
    ...
    private def tryRegisterAllMasters(): Array[JFuture[_]] = {
      // master可能有多个
      logInfo("=====4.StandAloneAppClient向所有的Master注册=====")
      for (masterAddress <- masterRpcAddresses) yield {
        ...
        registerMasterThreadPool.submit(new Runnable {
          override def run(): Unit = try {
            ...
            // 消息4 clientEndpoint向Master发送注册app的消息 跳转到Master
            logInfo("=====消息4 ClientEndPoint向Master发送RegisterApplication(注册App)消息=====")
    ...
    override def receive: PartialFunction[Any, Unit] = {
      ...
      case RegisteredApplication(appId_, masterRef) =>
        logInfo(s"=====处理消息5 Driver收到Master生成的appId：$appId_ =====")
      ...
      // 处理消息12 就是输出日志,到这里资源完成了，该退回SparkContext初始化的地方去进行任务调度了
