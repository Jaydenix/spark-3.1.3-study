...
private[deploy] class Master(
  ...
  override def receive: PartialFunction[Any, Unit] = {
    ...
    // 处理消息4 这里的driver就是clientEndpoint对象，它是StandaloneAppClient对象的内部类
    case RegisterApplication(description, driver) =>
      ...
      if (state == RecoveryState.STANDBY) {
        ...
        // 消息5 向driver发送app注册成功消息
        logInfo("=====消息5 Master向Driver发送RegisteredApplication(app注册成功)的消息=====")
        driver.send(RegisteredApplication(app.id, self))
        // 开始资源调度,创建driver和executor位置
        // 1.创建driver 2.创建executor
        logInfo("=====5.app注册成功之后，进行资源调度=====")
        schedule()
      }


    case ExecutorStateChanged(appId, execId, state, message, exitStatus) =>
      logInfo(s"=====master处理消息7 ExecutorStateChanged,appId=${appId},execId=${execId},state=${state}=====")
      ...
      execOption match {
        case Some(exec) =>
          ...
          // 更新executor的状态
          logInfo(s"=====7.master更新executor状态=====")
          exec.state = state
          ...
          logInfo(s"=====消息8 master向driver(ClientEndpoint)发送ExecutorUpdated更新消息,execId=${execId},state=${state}=====")
          exec.application.driver.send(ExecutorUpdated(execId, state, message, exitStatus, None))

          if (ExecutorState.isFinished(state)) {
            // Remove this executor from the worker and app
            logInfo(s"=====8.executor为FINISH状态 就会将其移除=====")
          ...
          // 只要有executor状态改变了，也就是资源的使用情况改变了，master都会再次进行资源调度
          logInfo(s"=====只要executor状态改变了，也就是资源的使用情况改变了，master都会再次进行资源调度=====")
          schedule()
        case None =>
    ...
    case DriverStateChanged(driverId, state, exception) =>
      logInfo("=====master处理消息4,更新driver状态=====")
      state match {
  ...
  override def receiveAndReply(context: RpcCallContext): PartialFunction[Any, Unit] = {
    //处理消息1，创建driver，description中携带了driver的启动命令，核心问题：driver放在哪？
    case RequestSubmitDriver(description) =>
      logInfo("=====Master处理消息1,开始创建driver=====")
      if (state != RecoveryState.ALIVE) {
        ...
        // master需要选择一个worker来创建driver，进
        // 该方法返回的是DriverInfo对象
        val driver = createDriver(description)
        persistenceEngine.addDriver(driver)
        // waitingDrivers为等待调度的driver集合
        waitingDrivers += driver
        drivers.add(driver)
        // 调度driver，选择driver的位置,并创建完毕
        logInfo("=====1. 新创建DriverInfo对象，开始Driver调度，也就是应该将Driver放在哪？=====")
        schedule()

        // TODO: It might be good to instead have the submission client poll the master to determine
        //       the current status of the driver. For now it's simply "fire and forget".
        //
        logInfo("=====消息3 master回应client 'SubmitDriverResponse'消息，表示master已经提交了创建driver的命令=====")
  ...
  private def scheduleExecutorsOnWorkers(
    ...
    logInfo("=====5.默认使用spreadOut策略，更有利于满足数据本地性，返回[worker,allocated cores]=====")
    // logInfo("=====5.如果没有指定executor cores,则每个executor 1core=====")
    ...
    if (freeWorkers.isEmpty) {
      logInfo(s"==========当前没有能运行一个至少executor的worker==========")
    } else {
      logInfo(s"==========能运行至少一个executor的Workers的索引数组===========")
      logInfo(s"==========${freeWorkers}==========")
      logInfo(s"=====================================")
    }
    ...
    while (freeWorkers.nonEmpty) {
      freeWorkers.foreach { pos =>
        ...
        while (keepScheduling && canLaunchExecutorForApp(pos)) {
          ...
          // 如果是spreadOut策略 那么就是分散地放置executor
          if (spreadOutApps) {
    ...
    if (assignedCores.isEmpty) {
      logInfo(s"===============没有消耗worker的资源===============")
    } else {
      logInfo(s"===============worker消耗的核心数===============")
      logInfo(s"=========${assignedCores.mkString("Array(", ", ", ")")}=========")
    }
    assignedCores
  }
  ...
  private def startExecutorsOnWorkers(): Unit = {
    ...
    if (waitingApps.isEmpty) logInfo("=====没有待调度的app,无需分配executor=====")
    for (app <- waitingApps) {
      logInfo(s"=====当前appId:${app.id} =====")
      logInfo("=====为app分配executor=====")
      val coresPerExecutor = app.desc.coresPerExecutor.getOrElse(1)
      // If the cores left is less than the coresPerExecutor,the cores left will not be allocated
      // 如果app需要的核心数<每个executor的核心数，就不会分配给app executor了
      if (app.coresLeft >= coresPerExecutor) {
        // Filter out workers that don't have enough resources to launch an executor
        // 过滤出可用的workers,并且根据可用核心的数目降序排序
        val usableWorkers = workers.toArray.filter(_.state == WorkerState.ALIVE)
          .filter(canLaunchExecutor(_, app.desc))
          .sortBy(_.coresFree).reverse
        if (usableWorkers.isEmpty) {
          logInfo("=====当前没有可用的worker=====")
        } else {
          logInfo("=====可用核心数由高到低,打印alive workers信息=====")
          logInfo("==================================")
        ...
        // 真正开始制定executor的放置策略，返回结果为分配核心的数组
        val assignedCores = scheduleExecutorsOnWorkers(app, usableWorkers, spreadOutApps)

        // Now that we've decided how many cores to allocate on each worker, let's allocate them
        if (usableWorkers.isEmpty) {
          logInfo("=====由于当前没有可用的worker,无法放置executor=====")
        } else {
          logInfo("=====将executor放置计划实现,从索引为0的worker开始依次放置executor=====")
        }

        for (pos <- 0 until usableWorkers.length if assignedCores(pos) > 0) {
          // 将计划付诸行动
          logInfo("=====以worker为单位真正放置executor=====")
  ...
  private def allocateWorkerResourceToExecutors(
    ...
    logInfo(s"=====在${worker.host}上消耗${assignedCores}个核,创建${numExecutors}个executor,每个executor${coresPerExecutor.get}个核=====")
    // 一个一个向worker发送启动executor的消息
    for (i <- 1 to numExecutors) {
      ...
      logInfo(s"将在${worker.host}上创建第${i}个executor,详细信息为${exec}")
      // 真正运行executor
  ...
  private def schedule(): Unit = {
    ...
    var curPos = 0
    // 如果在多用户条件下，同一时间需要创建的driver可能不止一个
    if (waitingDrivers.isEmpty) logInfo("=====当前没有待分配的Driver，直接进入executor资源调度=====")
    for (driver <- waitingDrivers.toList) {
      // iterate over a copy of waitingDrivers
      // We assign workers to each waiting driver in a round-robin fashion. For each driver, we
      // start from the last worker that was assigned a driver, and continue onwards until we have
      // explored all alive workers.
      logInfo("=====1.随机打乱worker,类似于spreadOut策略放置Driver=====")
      logInfo(s"=====当前放置的Driver是: $driver=====")
      var launched = false
      var isClusterIdle = true
      var numWorkersVisited = 0
      while (numWorkersVisited < numWorkersAlive && !launched) {
        // 取活着的workers中的第curPos个，这里是无论当前的worker能不能跑driver，都只访问一次，curPos++，再访问下一个
    ...
    // 只有当所有的driver创建完毕之后才会创建executor
    // logInfo("=====1/5.只有当所有的driver创建完毕之后才会创建executor,下面开始分配executor资源(做计划)=====")
  ...
  private def launchExecutor(worker: WorkerInfo, exec: ExecutorDesc): Unit = {
    ...
    // 消息6 master向要运行executor的worker发送启动executor的消息
    logInfo(s"=====消息6 Master向Worker发送运行executor的消息=====")
    ...
    // 消息12 master向driver发送executor启动成功的消息
  ...
  ApplicationInfo = {
    ...
    logInfo("=====4.根据date生成AppId,封装成ApplicationInfo对象=====")
  ...
  private def launchDriver(worker: WorkerInfo, driver: DriverInfo): Unit = {
    ...
    worker.addDriver(driver)
    driver.worker = Some(worker)
    // 消息2，向worker发送启动driver命令，跳转到worker
    logInfo("=====消息2 master向worker发送LaunchDriver消息=====")
