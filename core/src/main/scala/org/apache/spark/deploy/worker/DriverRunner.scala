...
private[deploy] class DriverRunner(
  ...
  private[worker] def start() = {
    //  创建线程运行driver，这里创建的线程并不是driver
    logInfo(s"=====2.创建DriverRunner for $driverId 线程=====")
    new Thread("DriverRunner for " + driverId) {
      override def run(): Unit = {
        ...
        logInfo(s"=====向启动Driver的worker自己发送DriverStateChanged消息 state=${finalState.get}=====")
  ...
  private[worker] def prepareAndRunDriver(): Int = {
    ...
    //真正运行driver
    logInfo("=====2.真正运行Driver=====")
    runDriver(builder, driverDir, driverDesc.supervise)
  }

  // builder中保存的是运行命令，也就是创建DriverWrapper进程，查看其main方法
  ...
  private[worker] def runCommandWithRetry(
    ...
    while (keepTrying) {
      ...
      synchronized {
        ...
        // 执行命令
        logInfo("=====2.执行创建DriverWrapper进程的命令=====")
        process = Some(command.start())
        initialize(process.get)
        logInfo("=====2.DriverWrapper初始化成功=====")
      }
