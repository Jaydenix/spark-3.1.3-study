...
private[deploy] class ExecutorRunner(
  ...
  private[worker] def start(): Unit = {
    // 同样创建运行executor的线程，和driver类似
    logInfo(s"=====创建运行executor的线程:ExecutorRunner for ${fullId}=====")
  ...
  private def fetchAndRunExecutor(): Unit = {
    try {
      ...
      // Launch the process
      // 准备命令 运行executor进程
      // 在standalone模式下，appDesc中包含了CoarseGrainedSchdulerBackend类的子类StandaloneSchedulerBackend
      // 与master通信时传递的创建executor的jar名称:org.apache.spark.executor.CoarseGrainedExecutorBackend
      ...
      val redactedCommand = Utils.redactCommandLineArgs(conf, command.asScala.toSeq)
        .mkString("\"", "\" \"", "\"")
      logInfo(s"=====6.构建运行org.apache.spark.executor.CoarseGrainedExecutorBackend进程的命令=====")
      ...
      // 真正开始运行启动executor的命令，得到CoarseGrainedExecutorBackend进程
      process = builder.start()
      logInfo(s"=====6.CoarseGrainedExecutorBackend进程启动完毕,进入该进程的main方法=====")
      ...
      logInfo(s"=====向worker自己发送消息7 ExecutorStateChanged,appId=${appId},execId=${execId},state=${state}=====")
      ...
      // 当executor的状态改变时，都会向worker发送executor状态改变的消息，此时state=ExecutorState.EXITED
