...
private[spark] class Executor(
  ...
  // 初始化线程池
  logInfo(s"=====10.初始化线程池threadPool=====")
  logInfo(s"=====10.Executor内容包括创建或杀死任务等操作=====")
  ...
  def launchTask(context: ExecutorBackend, taskDescription: TaskDescription): Unit = {
    // 同理，类似于DriverRunner ExecutorRunner,进,看看是如何得到真实的任务的数据的
    val tr = new TaskRunner(context, taskDescription, plugins)
    // 将任务放入待运行任务的集合中
    runningTasks.put(taskDescription.taskId, tr)
    // 放入线程池中执行
    logInfo(s"======12.创建TaskRunner线程,将其放入threadPool中,执行任务=====")
  ...
  class TaskRunner(
    ...
    override def run(): Unit = {
      ...
      // 向executorBackend发送 任务状态更新的消息 之后executorBackend向driver发送任务状态更新
      logInfo(s"=====正在运行任务taskId=${taskId}=====")
      ...
      try {
        ...
        // If this task has been killed before we deserialized it, let's quit now. Otherwise, off an abortTimer
