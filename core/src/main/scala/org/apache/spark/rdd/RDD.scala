 ...
 *  - A function for computing each split => 计算分区(分片)的函数
 *  - A list of dependencies on other RDDs => RDD的依赖
 *  - Optionally, a Partitioner for key-value RDDs (e.g. to say that the RDD is hash-partitioned)  => 分区器(数据应当如何分区，比如：按照奇偶?字符?)
 *  - Optionally, a list of preferred locations to compute each split on (e.g. block locations for
 *    an HDFS file) => 计算每个分片的位置偏好
 *
