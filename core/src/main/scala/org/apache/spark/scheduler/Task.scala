...
private[spark] abstract class Task[T](
  ...
  final def run(
    ...
    // 输入文件块管理器
    // This is used in HadoopRDD, FileScanRDD, NewHadoopRDD and InputFileName function in Spark SQL.
    ...
    try {
      // 运行任务，分为shuffleMapTask和ResultTask，真正要去取数据了
      runTask(context)
    } catch {
      case e: Throwable =>
