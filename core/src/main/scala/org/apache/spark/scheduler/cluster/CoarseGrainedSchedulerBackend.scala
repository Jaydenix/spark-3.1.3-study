...
class CoarseGrainedSchedulerBackend(scheduler: TaskSchedulerImpl, val rpcEnv: RpcEnv)
  ...
  class DriverEndpoint extends IsolatedRpcEndpoint with Logging {
    ...
    override def onStart(): Unit = {
      ...
      logInfo(s"=====3.CoarseGrainedSchedulerBackend 中的onStart方法中向自己发送ReviveOffers消息触发任务调度=====")

      reviveThread.scheduleAtFixedRate(() => Utils.tryLogNonFatalError {
        logInfo("\n \n")
        logInfo(s"###########CoarseGrainedSchedulerBackend会以reviveIntervalMs=${reviveIntervalMs}ms为周期发送" +
          s"ReviveOffers消息,触发任务调度,来保证延迟调度工作##########")
        Option(self).foreach(
          _.send(ReviveOffers)
        )
    ...
    override def receive: PartialFunction[Any, Unit] = {
      ...
      case LaunchedExecutor(executorId) =>
        ...
        // 同样会进行单个executor的任务调度
        logInfo(s"=====11.driver接收到LaunchedExecutor消息后 对executorId=${executorId}实施任务调度 此时executor才算真正创建完毕=====\n \n")
        makeOffers(executorId)
      case e =>
    ...
    override def receiveAndReply(context: RpcCallContext): PartialFunction[Any, Unit] = {
      ...
      // 处理消息9，executor向driver发送注册消息
      case RegisterExecutor(executorId, executorRef, hostname, cores, logUrls,
      attributes, resources, resourceProfileId) =>
        logInfo(s"=====driver处理消息9 RegisterExecutor=====")
        if (executorDataMap.contains(executorId)) {
          ...
          logInfo(s"=====9.监听executor=====")
          listenerBus.post(
            SparkListenerExecutorAdded(System.currentTimeMillis(), executorId, data))
          // Note: some tests expect the reply to come after we put the executor in the map
          // 消息8 driver回复executor true
          logInfo(s"=====消息10 driver向executor回复true,表示已经注册好了executor=====")
          context.reply(true)
        }
    ...
    private def makeOffers(): Unit = {
      ...
      // 只要任务在运行 就不能删除executor
      // 返回值为任务的描述信息，也就是任务应当分配到哪个executor上
      val taskDescs = withLock {
        // Filter out executors under killing
        // 获取活着的executor
        val activeExecutors = executorDataMap.filterKeys(isExecutorActive)
        // 进一步获取活着的executor的详细信息 比如 ID 空闲的core等等
        ...
        // 任务调度入口=====================================================================
        // 将executor资源分配计划制定好，返回值为任务的描述信息，也就是任务应当分配到哪个executor上
        logInfo(s"=====进入任务调度,isAllFreeResources=true=====")
        logInfo(s"=====传递的参数是:workOffers(executors)=${workOffers}=====")
        scheduler.resourceOffers(workOffers, true)
      }

      if (taskDescs.nonEmpty) {
        logInfo(s"###########################isAllFreeResources=true,任务调度的结果##############################")
        // 有任务调度成功
        if(taskDescs.head.nonEmpty) {
          // 任务调度完成之后 得到所有的任务描述信息之后 运行所有任务
          logInfo(s"=====taskDescs也就是将要放置在executors上的任务,是一个二维数组 每行表示executor[i]上分配的任务=====")
          logInfo(s"=====taskDescs的列数 = executor的核心数 / 每个task消耗的核心数=${taskDescs.head.length}=====")
          logInfo(s"=====taskDescs的行数 = 可用executor的数目 = ${taskDescs.length}======")
          logInfo(s"##################taskDescs=${taskDescs}#####################")
          logInfo(s"###########################isAllFreeResources=true,任务调度的结果,下面开始运行任务##############################\n \n \n")
        } else {
          logInfo(s"###########################isAllFreeResources=true,任务调度的结果,没有任务被调度##############################\n \n \n")
        }
    ...
    private def makeOffers(executorId: String): Unit = {
      ...
      val taskDescs = withLock {
        ...
        if (isExecutorActive(executorId)) {
          ...
          logInfo(s"=====11.executorId=${executorId},详细信息为${executorData}=====")
          ...
          // 只对单个executor进行任务调度
          logInfo(s"=====11.进一步封装成workOffers=${workOffers} =====")
          logInfo(s"=====11.进入TaskSchedulerImpl类中进行单个executor的任务调度 =====")
      ...
      if (taskDescs.nonEmpty) {
        logInfo(s"###########################isAllFreeResources=false,任务调度的结果##############################")
        // 有任务调度成功
        if (taskDescs.head.nonEmpty) {
          // 任务调度完成之后 得到所有的任务描述信息之后 运行所有任务
          logInfo(s"=====taskDescs也就是将要放置在executors上的任务,是一个二维数组 每行表示executor[i]上分配的任务=====")
          ...
          logInfo(s"=====taskDescs的行数 = 可用executor的数目 = ${taskDescs.length}======")
          logInfo(s"##################taskDescs=${taskDescs}#####################")
          logInfo(s"###########################isAllFreeResources=false,任务调度的结果,下面开始运行任务##############################\n \n \n")
        } else {
          logInfo(s"###########################isAllFreeResources=false,任务调度的结果,没有任务被调度##############################\n \n \n")
        }
        launchTasks(taskDescs)
      }
    }
    ...
    private def launchTasks(tasks: Seq[Seq[TaskDescription]]): Unit = {
      ...
      for (task <- tasks.flatten) {
        ...
        if (serializedTask.limit() >= maxRpcMessageSize) {
          ...
          // 消息12 launchTask，Driver给executorEndpoint发送运行任务的消息，
          // 参数为序列化后的任务的描述信息，而不是任务的真实数据
          logInfo(s"=====消息12 Driver向(executorId=${task.executorId},host=${executorData.executorHost})发送launchTask(启动任务)消息,参数为序列化后的任务======")
  ...
  // 向driver发送ReviveOffers消息，实际上就是给自己发
  override def reviveOffers(): Unit = Utils.tryLogNonFatalError {
    logInfo(s"=====任务从stage解析出来后,driver给自己发送ReviveOffers消息,表示开始任务调度,将任务分配给executor=====")
   ...
   */
  private def numExistingExecutors: Int = synchronized {
    executorDataMap.size
  }
