...
private[spark] class StandaloneSchedulerBackend(
  ...
  override def start(): Unit = {
    ...
    logInfo("=====3.client和cluster模式的区别在这里=====")
    logInfo("=====3.client mode:直接连接launcherBackend=====")
    logInfo("=====3.cluster mode:应用会提交到master，再连接launcherBackend=====")
    if (sc.deployMode == "client") {
      logInfo("=====3.当前为client mode:直接连接launcherBackend=====")
      launcherBackend.connect()
    }

    // The endpoint for executors to talk to us
    // 创建名叫CoarseGrainedScheduler的RpcEndpoint,说白了就是DriverEndpoint 用来和executor通信
    ...
    // 创建对象Command,其mainClass = org.apache.spark.executor.CoarseGrainedExecutorBackend,是用来启动executor的jar
    logInfo("=====3.构建启动org.apache.spark.executor.CoarseGrainedExecutorBackend的命令=====")
    ...
    logInfo("=====3.如果是动态资源分配 先将初始executor数设为0，之后master会获得真实的executor初始数=====")
    ...
    // 进一步封装成ApplicationDescription对象，内部包含了worker启动executor的命令
    logInfo("=====3.进一步将启动executor的命令,core,memo等信息封装成ApplicationDescription对象=====")
    val appDesc = ApplicationDescription(sc.appName, maxCores, sc.executorMemory, command,
      webUrl, sc.eventLogDir, sc.eventLogCodec, coresPerExecutor, initialExecutorLimit,
      resourceReqsPerExecutor = executorResourceReqs)
    // 创建rpc客户端对象
    logInfo("=====3.开始创建StandaloneAppClient客户端=====")
