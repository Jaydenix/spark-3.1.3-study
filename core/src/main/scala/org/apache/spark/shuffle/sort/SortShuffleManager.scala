...
private[spark] class SortShuffleManager(conf: SparkConf) extends ShuffleManager with Logging {
  ...
  override def registerShuffle[K, V, C](
    ...
    // 如果不需要map端预聚合,并且partitions的数目 < spark.shuffle.sort.bypassMergeThreshold(默认200)
    // BypassMergeSortShuffleHandle 直接将生成的文件合并为一个 这样避免了两次序列化和反序列化溢写文件
    if (SortShuffleWriter.shouldBypassMergeSort(conf, dependency)) {
      ...
      // 不需要map端预聚合 并且partition数目 > 16777215+1
  ...
  override def getWriter[K, V](
    ...
    /*
        ...
        根据shufflerhandler的类型来确定使用哪种shufflerWriter
        如果是
