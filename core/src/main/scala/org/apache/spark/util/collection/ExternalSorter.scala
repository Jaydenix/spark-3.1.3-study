...
private[spark] class ExternalSorter[K, V, C](
  ...
  def insertAll(records: Iterator[Product2[K, V]]): Unit = {
    ...
    // 如果需要 map 端的聚合操作，则使用 map 数据结构来保存计算结果数据
    if (shouldCombine) {
      // Combine values in-memory first using our AppendOnlyMap
      val mergeValue = aggregator.get.mergeValue
      val createCombiner = aggregator.get.createCombiner
      var kv: Product2[K, V] = null
      // update是一个函数 表示如何更新值 也就是如何启动map端预聚合比如 a->1 a->2 如果定义聚合逻辑为 new+old 则聚合之后得到 a->3
    ...
    // 如果不需要 map 端的聚合操作，则使用 buffer 数据结构来保存计算结果数据
    else {
  ...
  override protected[this] def spill(collection: WritablePartitionedPairCollection[K, C]): Unit = {
    // 对数据进行排序：排序器 KVArraySortDataFormat: TimSort 改良版本的 MergeSort
  ...
  : SpilledFile = {
    ...
    // 返回的是溢写文件 其中非常重要的是elementsPerPartition,也就是每个分区写了多少条记录
  ...
  def partitionedIterator: Iterator[(Int, Iterator[Product2[K, C]])] = {
    ...
    if (spills.isEmpty) {
      ...
      // 按照partitionId和key的顺序来合并一些文件和内存中的数据
  ...
  def writePartitionedMapOutput(
    ...
    // 如果溢写文件为空,也就是数据都在内存里面
    if (spills.isEmpty) {
      // Case where we only have in-memory data
      val collection = if (aggregator.isDefined) map else buffer
      // 按照partitionId排序内存中的数据
      val it = collection.destructiveSortedWritablePartitionedIterator(comparator)
      while (it.hasNext()) {
        val partitionId = it.nextPartition()
    ...
    // 溢写文件不为空 也就是说磁盘里有一部分数据 内存里也有一部分数据
    else {
      // We must perform merge-sort; get an iterator by partition and write everything directly.
      // this.partitionedIterator合并溢写文件和内存中的数据 返回的是按照partitionId排序的迭代器
      // (id, elements) id是分区编号 elements是分区的数据集
      for ((id, elements) <- this.partitionedIterator) {
